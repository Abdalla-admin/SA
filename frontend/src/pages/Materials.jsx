...
export default function Materials() {
  ...
  // Collect all unique categories including ones already in DB
  const allCats = [...new Set([...DEFAULT_CATS, ...items.map(m => m.category).filter(Boolean)])].sort();

  const openForm = (m = null) => {
    if (m) {
      const isCustom = m.category && !DEFAULT_CATS.includes(m.category);
  ...
  const del = async id => {
    if (!await confirm('Delete this material? This action cannot be undone.')) return;
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-center">
        ...
        <button onClick={() => openForm()} className="bg-orange-500 hover:bg-orange-600 text-white px-4 py-2 rounded-lg text-sm font-medium">+ Add Material</button>
      </div>
